...
echo "Converting to hex (using the fixed, byte-level-safe converter)..."
python convert.py "$BIN"
cp "${BIN%.bin}.hex" ./last_build.hex
echo "Saved a copy of the hex for inspection: ./last_build.hex"
echo "  (run: python3 hex_inspect.py last_build.hex)"

...
class InstructionDispatchQueueTest extends AnyFreeSpec with Matchers with ChiselSim {
    "IDQ add and pop instruction" in {
        simulate(new InstructionDispatchQueue()) { dut =>
            ...
            dut.io.broadcast_free_valid.poke(true.B)
            dut.io.broadcast_free_register.poke(1.U)
            dut.io.broadcast_free_value.poke(2.U)
            ...
            dut.io.broadcast_mark_valid.expect(true.B)
            dut.io.broadcast_mark_register.expect(3.U)
    ...
    "IDQ test simultaneous mark" in {
        simulate(new InstructionDispatchQueue()) { dut =>
            ...
            dut.io.valid.poke(true.B)
            dut.io.instruction.rs1.poke(3.U)

...
class MemoryInterfaceTest extends AnyFlatSpec with HasCliOptions with Cli.EmitVcd with ChiselSim {
  ...
  def doRead(dut: MemoryInferface, addr: Int, expectedData: Long, tag: String): Unit = {
    var readyCycles = 0
    while (!dut.io.ready.peek().litToBoolean && readyCycles < 20) {
      dut.clock.step()
      readyCycles += 1
    }
    assert(dut.io.ready.peek().litToBoolean, s"$tag: never got ready")

    dut.io.req.address.poke(addr.U)
    dut.io.req.write_data.poke(0.U)
    dut.io.req.read.poke(true.B)
    dut.io.req.write.poke(false.B)
    dut.io.start.poke(true.B)
    dut.clock.step()
    dut.io.start.poke(false.B)

    var cycles = 0
    while (!dut.io.valid.peek().litToBoolean && cycles < 20) {
      dut.clock.step()
      cycles += 1
    }
    // dut.clock.step()

    assert(dut.io.valid.peek().litToBoolean, s"$tag: never got valid")
    assert(dut.io.data.peek().litValue == BigInt(expectedData),
      s"$tag: expected 0x${expectedData.toHexString}, got 0x${dut.io.data.peek().litValue.toString(16)}")
    println(s"$tag PASS (ready after $readyCycles, valid after $cycles cycles)")
  }

  def doWrite(dut: MemoryInferface, addr: Int, data: Long, tag: String): Unit = {
    var readyCycles = 0
    while (!dut.io.ready.peek().litToBoolean && readyCycles < 20) {
      dut.clock.step()
      readyCycles += 1
    ...
    dut.io.req.address.poke(addr.U)
    dut.io.req.write_data.poke(data.U)
    dut.io.req.read.poke(false.B)
    dut.io.req.write.poke(true.B)
    dut.io.start.poke(true.B)
    dut.clock.step()
    dut.io.start.poke(false.B)
    ...
    assert(dut.io.ready.peek().litToBoolean, s"$tag: never got valid")
    println(s"$tag PASS (ready after $readyCycles, valid after $cycles cycles)")
  }

  it should "miss then hit on repeated read (cold miss + warm hit)" in {
    simulate(new MemoryInferface()) { dut =>
      dut.clock.step(2)
  ...
  it should "not assert valid without a request" in {
    simulate(new MemoryInferface()) { dut =>
      dut.io.start.poke(false.B)
      dut.clock.step(5)
      assert(!dut.io.valid.peek().litToBoolean, "valid should not fire without start")
    }
  }
  ...
  it should "write and read back 100 random addresses" in {
    simulate(new MemoryInferface()) { dut =>
      dut.clock.step(2)
      ...
      for (_ <- 0 until 100) {
        // pick a random word-aligned address within memory range
        val addr = (rng.nextInt(256) * 4) // 256 words, 4 bytes each
  ...
  it should "write all words in a line then evict and reload" in {
    simulate(new MemoryInferface()) { dut =>
      ...
      // reload and check all words survived writeback
